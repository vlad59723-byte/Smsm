...
const generatePromptHandler = async (req, res, next) => {
  try {
    ...
    if (operatingMode === 'no-names') {
        ...
        logger.info('Applying base64 filter (Universal)...');
        
        // РЕШЕНИЕ (Проблема 2: Короткий список):
        // Убираем проверку 'properNouns.includes(match)'.
        // Теперь ЛЮБОЕ слово, подходящее под regex (с заглавной буквы), будет закодировано.
